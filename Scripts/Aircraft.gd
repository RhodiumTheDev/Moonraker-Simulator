...
func _physics_process(delta):
	...
	for lifting_body in lifting_bodies:
		add_force(lifting_body.calculate_lift(delta, velocity, linear_velocity) * global_transform.basis.y, lifting_body.translation)
		pass
	
	if Input.is_action_pressed("pitch_down"):
		add_force(global_transform.basis.y*150, global_transform.basis.z*5)
	
	if Input.is_action_pressed("pitch_up"):
		add_force(global_transform.basis.y*-150, global_transform.basis.z*5)
	
	if Input.is_action_pressed("roll_CW"):
		add_force(global_transform.basis.y*150, global_transform.basis.x * -3.246)
	
	if Input.is_action_pressed("roll_ACW"):
		add_force(global_transform.basis.y*150, global_transform.basis.x * 3.246)
	
	if Input.is_action_pressed("yaw_left"):
		add_force(global_transform.basis.x*150, global_transform.basis.z*5)
